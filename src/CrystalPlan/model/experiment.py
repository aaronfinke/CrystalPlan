...
import utils
from experiment_calcs import init_vol_symm_map_cython, appl_vol_sym_cython, calc_coverage_stats_cython
...
class Experiment:
    ...
    def get_angles_to_measure_hkl(self, h, k, l, wl_tolerance):
        ...
        # res = scipy.optimize.minimize(_error_in_wavelength, x0, args, method='Nelder-Mead')
        best_detector_angle = res[0] #avoid error with 0-dimension array
    ...
    def fourcircle_measure_all_reflections(self, progress_dialog=None):
        ...
        for ref in self.reflections:
            ...
            timerlist.append(time.perf_counter()-t1)
        ...
        print(f"{len(self.inst.positions)=}")
        avgtime = np.average(timerlist)
        stdevtime = np.std(timerlist)
        print(f"avg time per iteration: {avgtime}, stdev: {stdevtime}")
        #We make the array of how many times measured, for all the positions
        self.get_reflections_times_measured(None)
...
class TestExperiment(unittest.TestCase):
    ...
    def setUp(self):
        ...
        instrument.inst = instrument.Instrument("../../../instruments/TOPAZ_detectors_2010_2.csv")
