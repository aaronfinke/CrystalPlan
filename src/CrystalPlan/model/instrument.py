...
from crystal_calcs import calculate_coverage_cython, get_coverage_cython, calculate_coverage_inelastic_cython
import goniometer
        ...
        try:
            with open(filename,"r") as fp:
                ...
                if row[0].startswith("#Cyl"):
                    cylindrical = True
            with open(filename,"r") as fp:
                reader= csv.reader(filter(lambda row: row[0]!='#' or len(row)==0, fp))
                count = 1
                row = next(reader)
                for row in reader:
...
class TestInelasticInstrument(unittest.TestCase):
    ...
    def setUp(self):
        ...
        tst_inst_path =Path("/Users/aaronfinke/CrystalPlan/instruments/TOPAZ_geom_all_2011.csv").as_posix()
...
class TestImagine(unittest.TestCase):
    """Unit test for the 4-circle class."""
    def setUp(self):
        tst_inst_path = Path("/Users/aaronfinke/CrystalPlan/instruments/IMAGINE_detectors.csv").as_posix()
