...
VALID_PARAMS = [PARAM_POSITIONS, PARAM_TRY_POSITION, PARAM_DETECTORS, PARAM_ENERGY_SLICE, PARAM_SLICE, PARAM_INVERT,
                PARAM_SYMMETRY, PARAM_DISPLAY, PARAM_REFLECTIONS, PARAM_REFLECTION_MASKING,
                PARAM_REFLECTION_HIGHLIGHTING, PARAM_REFLECTION_DISPLAY]
...
class Experiment:
    ...
    def __getstate__(self):
        ...
        exclude_list = ['reflections', 'reflections_dict', 'reflections_q_vector',
                        ...
                        'reflection_stats_with_symmetry', 'reflection_stats_adjusted',
                        'reflection_stats_adjusted_with_symmetry',
    ...
    def find_primary_reflections(self):
        ...
        print("Primary peaks: ", sum(primary), "out of", len(self.reflections),
              "peaks. Found in %.3f sec." % (time.time() - t1))
    ...
    def calculate_peak_shape(self, hkl, delta_hkl, poscov, det_num, num_points=100):
        ...
        hkl_array = np.vstack(
            (norm_funcs[0].rvs(num_points), norm_funcs[1].rvs(num_points), norm_funcs[2].rvs(num_points)))
    ...
    def calculate_reflections_mask(self):
        ...
        assert len(self.reflections_times_measured) == len(
            self.reflections), "The list of reflections_times_measured should be the same length as the # of reflections."
        assert len(self.reflections_times_measured_with_equivalents) == len(
            self.reflections), "The list of reflections_times_measured_with_equivalents should be the same length as the # of reflections."
    ...
    def get_angles_to_measure_hkl(self, h, k, l, wl_tolerance):
        ...
        def _error_in_wavelength(detector_angle, return_rot):
            ...
            (rot, wavelength) = crystal_calc.get_sample_rotation_matrix_to_get_beam(beam_wanted, hkl, ub_matrix,
                                                                                    starting_rot_matrix=None)
            # Error in wavelength
    ...
    def _friendly_time(self, seconds):
        ...
        try:
            ...
            if ages > 0:
                return "%d age%s of the universe, %d year%s, %d week%s, %d day%s and %0dh %02dm %02ds" % (
                ages, ['', 's'][ages > 0], y, ['', 's'][y > 0], w, ['', 's'][w > 0], d, ['', 's'][d > 0], h, m, s)
            elif y > 0:
                return "%d year%s, %d week%s, %d day%s and %0dh %02dm %02ds" % (
                y, ['', 's'][y > 0], w, ['', 's'][w > 0], d, ['', 's'][d > 0], h, m, s)
            elif w > 0:
    ...
    def estimated_time_string(self):
        ...
        if pcharge > 0:
            ...
            s += "%.3e pC proton charge, approx %s (at 1 MW accelerator power)." % (
            pcharge, self._friendly_time(pcharge / pC_per_second_at_100kw))
        if other > 0:
            if len(s) > 0: s += "; and an "
    ...
    def calculate_coverage(self, pos_param=None, det_param=None):
        ...
        if isinstance(self.inst, instrument.InstrumentInelastic):
            ...
            if slice is None:
                ...
                self.qspace = self.inst.total_coverage(detectors, positions_used, slice_min=slice.slice_min,
                                                       slice_max=slice.slice_max)
    ...
    def initialize_volume_symmetry_map(self):
        ...
        if self.verbose: print(
            "Starting volume symmetry calculation. Order is %d. Matrix is %d voxels (%d to a side)." % (
            order, n ** 3, n))
        ...
        if config.cfg.force_pure_python:
            ...
            symm = self.map_voxels_jit(B,invB,symm,qres,qlim,n,order,table)
    ...
    @staticmethod
    @jit
    def map_voxels_jit(B,invB,symm,qres,qlim,n,order,table):
        for ix in range(n):
            qx = ix*qres - qlim
            for iy in range(n):
                qy = iy * qres - qlim
    ...
    def apply_volume_symmetry(self, use_inline_c=True):
        ...
        if use_inline_c and not config.cfg.force_pure_python:
            ...
            qspace_flat = self.apply_volume_symmetry_jit(old_q, qspace_flat, numpix, order, symm)
    ...
    @staticmethod
    @jit(nopython=True)
    def apply_volume_symmetry_jit(old_q, qspace_flat, numpix, order, symm):
        for pix in range(numpix):
            for ord in range(order):
                index = symm[pix, ord]
                if index >= 0:
    ...
    def slice_coverage(self):
        ...
        if slice.use_slice:
            ...
            if qmin <= 0:
                ...
                self.qspace_displayed = self.qspace_displayed * (
                            (self.inst.qspace_radius < qmax) & (self.inst.qspace_radius > qmin))
            self._lock_qspace_displayed.release()
    ...
    def calculate_coverage_stats(self):
        ...
        if config.cfg.force_pure_python:
            ...
            (results, total_points, covered_points0, covered_points1,
             covered_points2, covered_points3) = self.calculate_coverage_stats_jit(qspace, qspace_radius, q_step,
                                                                                   qlim, total_points, qspace_size, num,
                                                                                    covered_points0, covered_points1,
                                                                                   covered_points2, covered_points3)
            overall_points, overall_covered_points, overall_redundant_points = results
            # Overall stats
    ...
    @staticmethod
    @jit(nopython=True)
    def calculate_coverage_stats_jit(qspace, qspace_radius, q_step, qlim, total_points, qspace_size, num,
                                     covered_points0, covered_points1, covered_points2, covered_points3):
        overall_points = 0
        overall_covered_points = 0
        overall_redundant_points = 0

        for i in range(qspace_size):
            # coverage value at this point
            val = qspace[i]
            ...
            if (slice < num) and (slice >= 0):
                total_points[slice] += 1
                if val > 0:
                    covered_points0[slice] += 1
                    if val > 1:
                        covered_points1[slice] += 1
                        if val > 2:
                            covered_points2[slice] += 1
                            if val > 3:
                                covered_points3[slice] += 1
        results = (overall_points, overall_covered_points, overall_redundant_points)
        return results, total_points, covered_points0, covered_points1, covered_points2, covered_points3
    ...
    def calculate_reflection_coverage_stats(self, edge_avoidance=False, edge_x=0, edge_y=0):
        ...
        assert len(self.primary_reflections_mask) == len(
            self.reflections), "The primary reflections mask should be the same length as the reflections; otherwise, incorrect stats will be computed."
        assert len(self.reflections_times_measured_with_equivalents) == len(
            self.reflections), "reflections_times_measured_with_equivalents is the right length."
        mask = self.primary_reflections_mask
        if len(mask) > 0:
            self.reflection_stats_with_symmetry.total = np.sum(mask)
            self.reflection_stats_with_symmetry.measured = np.sum(
                self.reflections_times_measured_with_equivalents[mask, :] > 0)
            self.reflection_stats_with_symmetry.redundant = np.sum(
                self.reflections_times_measured_with_equivalents[mask, :] > 1)
...
class TestExperiment(unittest.TestCase):
    ...
    def test_initialize_reflections_automatic(self):
        ...
        e.initialize_reflections()
        assert len(
            e.reflections) == 125, "Correct # of reflections (we got %d), automatic mode, not limited to sphere" % len(
            e.reflections)
        e.range_h = (-10, 10)
        e.range_k = (-10, 10)
        e.range_l = (-10, 10)
    ...
    def continue_get_reflections_measured(self, measured, num_expected, message):
        ...
        assert len(
            l) == num_expected, "get_reflections_measured returns the right # of refls (we wanted %d, got %d)%s" % (
        num_expected, len(l), message)
        if num_expected > 0:
    ...
    def test_reflection_masking(self):
        ...
        e.calculate_reflections_mask()
        assert e.reflections_q_vector[:, e.reflections_mask].shape == (
        3, num), "Masking applied to array works, even with no masking."
        assert e.reflections_times_measured[e.reflections_mask, :].shape == (
        num, 1), "Masking applied to array works, even with no masking."
    ...
    def _check_get_equivalent_reflections(self, hkl, hkl_list):
        ...
        assert len(found) == len(hkl_list) + 1, "correct # of results for %s. We wanted %d results but got %d" % (
        message, len(hkl_list), len(found))
    ...
    def test_volume_symmetry(self):
        ...
        assert np.allclose(qspace_c,
                           qspace_python), "Volume coverage, with symmetry, is identical when made with C and Python"
