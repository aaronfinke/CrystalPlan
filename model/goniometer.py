...
from traitsui.api import View, Item, Group, Label, Heading, Spring, Handler, TupleEditor, TabularEditor, ArrayEditor, \
    TextEditor, CodeEditor, ListEditor
...
class AngleInfo(HasTraits):
    ...
    conversion = Float(label='Internal to friendly unit conversion factor',
                       desc='that multiplying internal units by this value gives you the friendly units')
    das_units = String(label='DAS units', desc='the units required by the DAS group (used in the output CSV file).')
    das_conversion = Float(label='Internal to DAS unit conversion factor',
                           desc='that multiplying internal units by this value gives you the DAS units')
    friendly_range = List(Float, label='Range of values in GUI (friendly units)',
                          desc='two numbers giving the minimum and maximum values to show in GUI, for the sliders for example.')
    ...
    def __str__(self):
        return "%s, %s, ranges from %.1f to %.1f %s" % (
        self.name, self.type, self.friendly_range[0], self.friendly_range[1], self.friendly_units)
...
class Goniometer(HasTraits):
    ...
    wl_angles = List(AngleInfo, label='Wavelength control parameters',
                     desc="the list of wavelength control parameters.", rows_trait=1)

    angles_desc = Property(String, depends_on=["gonio_angles", "wl_angles", "wavelength_bandwidth"],
                           label='Description of Angles:',
                           desc="a description of the angles/other positions controlled by the goniometer")

    def _get_angles_desc(self):
        return self.get_angles_description()

    wavelength_bandwidth = Property(Float,
                                    desc="the bandwidth of measurement wavelength, in angstroms. Changing this value changes the allowable range for WL_center.")
    ...
    wavelength_minimum = Property(Float,
                                  desc="the minimum wavelength that can be reached, in angstroms. Changing this value changes the allowable range for WL_center.")
    ...
    wavelength_maximum = Property(Float,
                                  desc="the maximum wavelength that can be reached, in angstroms. Changing this value changes the allowable range for WL_center.")
    ...
    def csv_add_position(self, fileobj, angle_values, count_for, count_value, comment):
        ...
        if not allowed:
            ...
            fileobj.write(
                "#"" ----- ERROR! This sample orientation could not be achieved with the goniometer, because of '%s'. THE FOLLOWING LINE HAS BEEN COMMENTED OUT ------ ""\n" % reason)
...
class LimitedGoniometer(Goniometer):
    ...
    def _angle_fitness_python(self, rot_angle, initial_rotation_matrix, ending_vec, starting_vec):
        ...
        if False:
            ...
            assert np.allclose(result_vec, ending_vec,
                               atol=1e-5), "rotated rotation matrix still makes the correct rotation. Expected %s, got %s" % (
            ending_vec, result_vec)
    ...
    def _angle_fitness_brute(self, rot_angle_list, initial_rotation_matrix, ending_vec, starting_vec):
        ...
        {
            rot_angle544  = ROT_ANGLE_LIST1(angle_num);
            //printf("angle of %e\\n", rot_angle);
        ...
        phi_list = []
        omega_list = []
        args = self.get_fitness_function_c_code()
        # Prepare variables, run the C code
        fitnesses, phi_list, chi_list, omega_list = self._angle_fitness_brute_jit(args, rot_angle_list, ending_vec,
                                                                                  initial_rotation_matrix,
                                                                                  fitnesses, chi_list, phi_list,
                                                                                  omega_list)
        # Test that the resulting matrix is still OK
        # if False:
    ...
    @staticmethod
    @jit(nopython=True)
    def _angle_fitness_brute_jit(args, rot_angle_list, ending_vec, initial_rotation_matrix,
                                 fitnesses, chi_list, phi_list, omega_list):
        for rot_angle in rot_angle_list:
            ...
            total_rot_matrix = np.zeros((3, 3))
            for i in range(3):
                for j in range(3):
                    for k in range(3):
                        total_rot_matrix[i][j] += extra_rotation_matrix[i][k] * initial_rotation_matrix(k, j)

            ux, uy, uz, vx, vy, vz, nx, ny, nz = total_rot_matrix.flatten()
            ...
            # try the original angles
            fitness = LimitedGoniometer.fitness_function(args, phi, chi, omega)
            fitnesses.append(fitness)
            phi_list.append(phi)
            chi_list.append(chi)
            omega_list.append(omega)

            # make angles closer to zero
            phi -= 2 * pi if phi > pi else 0
            chi -= 2 * pi if chi > pi else 0
            omega -= 2 * pi if omega > pi else 0
            phi += 2 * pi if phi < -pi else 0
            chi += 2 * pi if chi < -pi else 0
            omega += 2 * pi if omega < -pi else 0
            fitness = LimitedGoniometer.fitness_function(args, phi, chi, omega)
            ...
            chi_list.append(chi)
            omega_list.append(omega)
        return fitnesses, phi_list, chi_list, omega_list

    @staticmethod
    @jit(nopython=True)
    def fitness_function(args, phi, chi, omega) -> float:
        phi_min = args[0]
        phi_max = args[1]
        chi_min = args[2]
        chi_max = args[3]
    ...
    def calculate_angles_to_rotate_vector(self, starting_vec, ending_vec, starting_angles=None, search_method=0):
        ...
        if not np.any(np.isnan(result_vec)):
            assert np.allclose(result_vec,
                               ending_vec), "initial rotation matrix makes the correct rotation. Got %s, expected %s" % (
            result_vec, ending_vec)
        ...
        def optimize_c_code(start, stop, step):
            ...
            # Go through every angle
            rot_angle_list = np.arange(start, stop, step)
            (best_rot_angle, best_angles) = self._angle_fitness_brute(rot_angle_list, initial_R, ending_vec,
                                                                      starting_vec)
            return (best_rot_angle, best_angles)
        ...
        if search_method:
            ...
            if False:
                x0 = best_rot_angle
                res = scipy.optimize.fminbound(self._angle_fitness, 0, 2 * pi, args, xtol=4e-3, disp=0, maxfun=100,
                                               full_output=0)
                best_rot_angle = res
            else:
                x0 = np.array([best_rot_angle])
                res = scipy.optimize.fmin(self._angle_fitness_python, x0, args, xtol=4e-3, ftol=1e-2, disp=0,
                                          maxiter=100)
...
class MandiGoniometer(LimitedGoniometer):
    ...
    # Omega is 90
    omega = Float(+90.0, label="Fixed Omega angle (deg)",
                  desc="the fixed Omega angle that the goniometer has, in degree.")
    ...
    def calculate_angles_to_rotate_vector(self, *args, **kwargs):
        ...
        if best_angles is None:
            ...
            if not np.abs(chi - np.deg2rad(self.chi)) < 0.1 / 57 and not np.abs(
                    omega - np.deg2rad(self.omega)) < 0.1 / 57:
...
class ImagineMiniKappaGoniometer(LimitedGoniometer):
    ...
    # Alpha is 24 degrees
    alpha = Float(24.0, label="Fixed Alpha angle (deg)",
                  desc="the fixed Chi angle that the goniometer has, in degrees.")
...
class TopazInHouseGoniometer(LimitedGoniometer):
    ...
    def calculate_leg_xy_limits(self, visualize=False):
        ...
        if visualize:
            ...
            pylab.hold(True)
            for leg in range(3):
                pylab.pcolor(self.leg_safe_xaxis + fixed_plate_zero[COORD_X, leg],
                             self.leg_safe_zaxis + fixed_plate_zero[COORD_Z, leg],
                             self.leg_safe_zone[leg, :, :].transpose())
            pylab.xlabel("x")
            pylab.ylabel("z")
    ...
    def calculate_allowable_angles(self, angular_resolution=0.01, visualize=0, optimized=False):
        ...
        if visualize == 1:
            ...
            for i_phi in range(self.phi_list.size):
                ...
                if visualize:
                    pylab.figure(0)
                    pylab.imshow(self.allowed[i_phi, :, :], interpolation='nearest',
                                 extent=(-self.omega_range, self.omega_range, -self.chi_range, self.chi_range))
                    pylab.xlabel("Omega")
                    pylab.ylabel("Chi")
    ...
    def plot_goniometer(self, first_plot=False):
        ...
        if abs(error) > 0.01:
            warning(
                'The middle of the sample plate\'s 3 points does not match the middle calculated with sample motor rotation!\nError is %f' % difference)

        if vector_length(self.pin) > 0.01:
            warning(mcat([mstring('The calculated sample position is not zero after rotation/translation!'), 10,
                          mstring('This means that the calculation failed to keep the sample centered, somehow.'), 10,
                          mstring('Sample is at: '), num2str(self.pin.cT)]))

        error = vector_length(self.motor - self.sample_middle) - self.motor_vector_length
        if abs(error) > 0.01:
            warning(mcat([mstring(
                'The calculated length of the motor vector after rotation does not match the expected length!'),
                          mstring('Error is '), num2str(error)]))
        ...
        for leg in range(3):
            ...
            self._plot_legs[leg] = arrow(self.sample_plate[:, leg], self.fixed_plate[:, leg], color=color,
                                         plotobj=self._plot_legs[leg])
    ...
    def csv_make_header(self, fileobj, title, comment=""):
        ...
        # Line of header info
        fileobj.write(csv_line(
            ['Phi', 'LegA_X', 'LegA_Y', 'LegB_X', 'LegB_Y', 'LegC_X', 'LegC_Y', 'CountFor', 'CountValue', 'Comment']))
    ...
    def csv_add_position(self, fileobj, angles, count_for, count_value, comment):
        ...
        angle_comment = '#"Equivalent goniometer angles (phi, chi, omega) are: ' + string.join(
