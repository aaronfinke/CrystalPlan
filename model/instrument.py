...
from .numpy_utils import column, rotation_matrix, vector_length, normalize_vector, index_evenly_spaced, \
    az_elev_direction
...
from . import config
from . import utils
from model.crystal_calc import getq_jit, getq_inelastic_jit
...
class Instrument:
    ...
    def __setstate__(self, d):
        ...
        for poscov in self.positions:  # @type poscov PositionCoverage
            poscov.coverage = self.calculate_coverage(self.detectors, poscov.angles, poscov.sample_U_matrix,
                                                      use_inline_c=True)
    ...
    def make_blank_qspace(self, datatype, number_of_ints=None):
        ...
        if self.qx_list is None:
            warnings.warn(
                "instrument.make_blank_qspace(): called before instrument was properly initialized. Call make_qspace() first!")
            return None
    ...
    def calculate_coverage(self, det_list, angles, sample_U_matrix=np.identity(3), use_inline_c=True):
        ...
        if not (len(angles) == len(self.angles)):
            raise ArgumentError(
                "You supplied a list of angles of a length that does not match what the instrument is setup for.")
        ...
        for det in det_list:
            ...
            messages.send_message_optional(self, messages.MSG_UPDATE_MAIN_STATUSBAR,
                                           "Calculating coverage of detector '%s' at %s" % (det.name, angles_string))
            ...
            if use_inline_c and not config.cfg.force_pure_python:
                ...
                coverage = Instrument.get_coverage_jit(xlist, ylist, azimuthal_angle, elevation_angle,
                         rot_matrix, set_value1, set_value2, number_of_ints,
                         coverage, stride, max_index, wl_min, wl_max, self.qlim, self.q_resolution)
            else:
                # -------- Pure Python ---------
                for ix in xlist:
                    for iy in ylist:
    ...
    @staticmethod
    @jit(nopython=True)
    def get_coverage_jit(xlist, ylist, azimuthal_angle, elevation_angle,
                         rot_matrix, set_value1, set_value2, number_of_ints,
                         coverage, stride, max_index, wl_min, wl_max, qlim, q_resolution):
        # loop throuhg pixels using the lists given before.
        for ix in xlist:
            for iy in ylist:
                # angles of the detector pixel positions
                ...
                # get the two limits
                q_min = getq_jit(wl_min, elevation=elev, azimuthal_angle=az, rot_matrix=rot_matrix)
                q_max = getq_jit(wl_max, elevation=elev, azimuthal_angle=az, rot_matrix=rot_matrix)
                # limit them to the modeled size
                q_min = Instrument.shrink_q_vector_jit(q_min, qlim)
                q_max = Instrument.shrink_q_vector_jit(q_max, qlim)

                # find out how long of a line that is
                q_length = Instrument.vector_length(q_max) - Instrument.vector_length(q_min)
                if q_length > 0:
                    q_length = q_length * -1
                ...
                if numfrac > 0: 
                    ...
                    # size in q-space of each step
                    dx = float((q_max[0] - q_min[0])) / numfrac
                    dy = float((q_max[1] - q_min[1])) / numfrac
                    dz = float((q_max[2] - q_min[2])) / numfrac

                    lim_min = float(-qlim)
                    q_min_x, q_min_y, q_min_z = float(q_min)
    ...
    @staticmethod
    @jit(nopython=True)
    def shrink_q_vector_jit(q, limit):
        length = Instrument.vector_length(vector=q)
        if length <= 0:
            return q
    ...
    def simulate_position(self, angles, sample_U_matrix=np.identity(3), use_multiprocessing=False):
        ...
        messages.send_message_optional(self, messages.MSG_UPDATE_MAIN_STATUSBAR,
                                       "Calculating %s%s..." % (angles_string, ump))
        ...
        # Statusbar update
        messages.send_message_optional(self, messages.MSG_UPDATE_MAIN_STATUSBAR,
                                       "Calculation of %s complete." % angles_string)
    ...
    def total_coverage(self, detectors_used, orientations_used, use_inline_c=True):
            ...
            for i in range(len(detectors_used)):
                if detectors_used[i]:
                    if i < 62:
                        ...
                        print(
                            "cannot compute total_coverage for detector #%d as it exceed the max of 62 detectors." % i)
        ...
        if config.cfg.force_pure_python or not use_inline_c:
            ...
            coverage = Instrument.total_coverage_jit(number_of_ints, coverage, mask1, mask2, coverage_list)
    ...
    @staticmethod
    @jit(nopython=True)
    def total_coverage_jit(number_of_ints, coverage, mask1, mask2, coverage_list):
        for one_coverage in coverage_list:
            # By applying the mask and the >0 we take away any unwanted detectors
            if number_of_ints == 1:
                coverage += ((one_coverage[:, :, :, 0] & mask1) != 0)
...
class InstrumentInelastic(Instrument):
    ...
    def calculate_coverage(self, det_list, angles, sample_U_matrix=np.identity(3), use_inline_c=True):
        ...
        for det in det_list:
            ...
            if use_inline_c and not config.cfg.force_pure_python:
                ...
                varlist += attribute_list
                # Run the C code (see between function declarations for the actual code).
                coverage = self.get_coverage_inelastic_jit(wl_input, xlist, ylist, azimuthal_angle, elevation_angle,
                                           rot_matrix, energy_constant, ki_squared, ki,
                                           coverage, stride, max_index, wl_min, wl_max, self.qlim, self.q_resolution)
                ...
                print("Python-code: neutron energy gain min", np.min(coverage), "; max",
                      np.max(coverage[coverage < 1e6]))
    ...
    @staticmethod
    @jit(nopython=True)
    def get_coverage_inelastic_jit(wl_input, xlist, ylist, azimuthal_angle, elevation_angle,
                                    rot_matrix, energy_constant, ki_squared, ki,
                                    coverage, stride, max_index, wl_min, wl_max, qlim, q_resolution):
        lim_min = -qlim
        lim_max = qlim
        for ix in xlist:
            for iy in ylist:
                az = azimuthal_angle[iy,ix]
                elev = elevation_angle[iy,ix]
                q_max_both = getq_inelastic_jit(wl_input, wl_min, elev, az, rot_matrix)
                q_min_both = getq_inelastic_jit(wl_input, wl_max, elev, az, rot_matrix)

                q_max_length = 0
                q_min_length = 0
                q_max = q_min = q_max_unrot = q_min_unrot = [0.0, 0.0, 0.0]
                for i in range(3):
                    q_max[i] = q_max_both[i]
                    q_max_unrot[i] = q_max_both[i+3]
                    q_max_length +=  q_max[i] * q_max[i]
                ...
                q_max_length = sqrt(q_max_length)
                q_min_length = sqrt(q_min_length)
                q_diff = q_diff_unrot = [0.0, 0.0, 0.0]
                q_length = 0
                for i in range(3):
                    # We change the sign, because for inelastic, Q = ki-kf, rather than the opposite sign for elastic
                    q_diff[i] = q_max[i] - q_min[i]
                    ...
                    for i in range(numfrac):
                        ...
                        if (iqx >= 0) and (iqx < stride):
                            ...
                            if (iqy >= 0) and (iqy < stride):
                                qz = q_min[2] + i * dz
                                qz_unrot = q_min_unrot[2] + i * dz_unrot
                                iqz = int(round((qz - lim_min) / q_resolution))
                                if (iqz >= 0) and (iqz < stride):
                                    # calculate the neutron energy gain
                                    # but we need to get back the z component of kf
                                    # we kept Q = kf - ki
    ...
    def total_coverage(self, detectors_used, orientations_used, use_inline_c=True, slice_min=-100.0, slice_max=100.0):
        ...
        if config.cfg.force_pure_python or not use_inline_c or True:
            ...
            coverage = self.total_coverage_jit(coverage, num_coverage, coverage_list, slice_min, slice_max)
    ...
    @staticmethod
    @jit(nopython=True)
    def total_coverage_jit(coverage, num_coverage, coverage_list, slice_min, slice_max):
        for one_coverage in coverage_list:
            # Add one to the voxels where the energy is within the given range.
            coverage += (one_coverage[:, :, :] >= slice_min) & (one_coverage[:, :, :] <= slice_max)
        return coverage
...
class TestInelasticInstrument(unittest.TestCase):
    ...
    def test_simulate_position(self):
        ...
        print(cov.size)
        assert total_measured == total_coverage, "Measured the same # of voxels. We found %d in total_coverage, vs %d in the individual coverage" % (
        total_measured, total_coverage)
...
class TestInstrumentWithDetectors(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        assert len(
            empty_instrument.detectors) == 0, "Create an Instrument with a file not found, and you get no error but an empty detectors list."
        ...
        assert np.allclose(tst_inst.qspace_radius[0, 0, 0],
                           np.sqrt(3 * tst_inst.qlim ** 2)), "qspace_radius value tested"
    ...
    def do_calculate_coverage(self, more_det=False):
        ...
        assert len(
            x) < 20, "Coverage calculated by C and python match within 20 differences. We found %s differences" % len(x)
        # Do a full list at 0
        ret = tst_inst.calculate_coverage(tst_inst.detectors, [0, 0, 0], use_inline_c=True)
        found = np.sum(ret > 0)
        wanted = 55870
        if more_det: wanted = 167077
        assert found == wanted, "%d points found with %d detectors. We expected %d points" % (
        found, len(tst_inst.detectors), wanted)
    ...
    def test_simulate_and_total(self):
        ...
        (valid, redundant, invalid, invalid_reason) = tst_inst.evaluate_position_list([phi_list, [0], [0]],
                                                                                      ignore_gonio=True)
        ...
        cov_sum = np.sum(cov)
        expected = 223107
        assert cov_sum == expected, "total_coverage() total should be %d for these settings, but we got %s." % (
        expected, cov_sum)
        assert np.sum(
            cov) == total, "total_coverage() total (%s) should match the total those of each individual position (%s)." % (
        cov_sum, total)
        ...
        assert np.all(
            cov == cov_python), "total_coverage() gives the same using pure Python as with inline C. There are %d differences." % (
            np.nonzero(cov == cov_python)[0].size)
